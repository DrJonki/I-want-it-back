...
public:
	...
	void loadSheet(sf::Image &sheet,		//A reference to the source image
				   ...
				   const int _frameCount);	//Number of frames to load. Must be exact. Missing textures will be replaced with a white box.
	...
	void setCurrentFrame(unsigned int _currentFrame); //Set the current frame. Can be used to reset the animation.

	bool lastFrame(); //Returns true if the current frame is the last one.

	void setStepInterval(unsigned int _stepInterval); //Set the step interval. For example, a value of 2 means the frame gets updated every second step call.
	...
	sf::Texture getCurrentTexture(); //Returns a reference to the current frame texture.
